...
def CreateTheCmdFile():
    ...
    if useLSF:
        ...
        #else:
            #cmd_file.write('+JobFlavour             = "longlunch"\n')
...
def AddJobToCmdFile():
    ...
    if useLSF:
        cmd_file.write("bsub -q " + Jobs_Queue + " -J " + Jobs_Name+Jobs_Index + " -o " + Path_Log + ".out -e " + Path_Log + ".err '"+os.getcwd()+"/" + Path_Shell + "'\n")
    else:
        cmd_file.write('\n')
...
def SendCluster_Create(FarmDirectory, JobName):
    ...
    #command_out = commands.getstatusoutput("condor_q")[1]
...
def SendCluster_Submit():
    ...
    if useLSF:
        print 'Launching the LXBATCH command file in', Path_Cmd
